...
```{r, echo = FALSE}
y <- read_csv("ps_4_elections-poll-nc09-3.csv") %>% 
  ...
  gt() %>% 
  tab_header(
    title = "Polling Results in NC 9th Congressional District") %>% 
...
e <- read_csv("ps_4_elections-poll-nc09-3.csv") %>% 
  ...
  filter(educ4 %in% c("Graduate or Professional Degree", "Bachelors' Degree",
                                  "Some college or trade school", "High School",
                                  "Grade School")) %>%
  ## We filter out to only the course departments that we are interested in for this graph
  group_by(educ4) %>% 
  select(educ4, final_weight) 
e %>% 
  ggplot(aes(x = final_weight, y = educ4)) +
  ...
  scale_fill_viridis(option = "B", guide = FALSE) +
  ...
  labs(title = "More Educated Matter Less in North Carolina 9th", 
       subtitle = "Poll gives more weight to people who are less likely to participate in polls", caption = "New York Times Upshot/Siena College 2018 live polls")
## Assigns proper titles, subtitles, and captions
```
